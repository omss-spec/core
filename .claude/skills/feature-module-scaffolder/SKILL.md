...
- JSDoc on every exported interface, its methods, and every exported function, following AGENTS.md's JSDoc/TSDoc standard: one-line imperative summary ending in a period, `@typeParam` for every generic, `@param`/`@returns` in signature order (`@returns` omitted for `void`), `@example` on primary public entry points, and `@dangerous` (never a new ad-hoc tag) for footguns.
