...
/**
 * Creates a new {@link ProviderRegistry}.
 */
export function createProviderRegistry(): ProviderRegistry {
    /**
     * Internal map of registered providers, keyed by provider ID.
     */
    const providers = new Map<string, UnknownProvider>()

    function getAll(filter?: (p: UnknownProvider) => boolean): UnknownProvider[] {
        if (filter) {
            return [...providers.values()].filter(filter)
        }
    ...
    return {
        async add(provider) {
            ...
            const valResolverNamespaceReq = validateSafeUniqueString(
                provider.resolver.namespace,
                'resolver namespace for resolver with name: "' + provider.resolver.name + '" and id:',
                OMSSProviderError
            )
            ...
            if (getAll().some((p) => p.resolver.namespace === provider.resolver.namespace && p.resolver !== provider.resolver)) {
